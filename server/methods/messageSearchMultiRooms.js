...
Meteor.methods({
	messageSearchMultiRooms(text, roomList, limit) {
		function messageSearch(text, rid, limit) {
			check(text, String);
			check(rid, String);
		...
		for (room in roomList) {
			temp = messageSearch(text, roomList[room], limit);
			console.log('temp');
			console.log(temp);
			result.messages = result.messages.concat(temp.messages);
			result.users = result.users.concat(temp.users);
			result.channels = result.channels.concat(temp.channels);
			console.log('result');
			console.log(result);
		}
		return result;
